...
[ -d "geometries/unoptimized/" ] || printf "\nThe ./geometries/unoptimized/ \
directory doesnt exist. \n\n" # checks to see if the directory containing
# the unoptimized geometries of the conformers exists - exits on return 1

cd "geometries/unoptimized/" # enters that directory

ls -l *.xyz &>/dev/null || printf "\nNo .xyz files detected in the \
./geometries/unoptimized/ directory. \n\n" # lists all files ending in .xyz 
...
echo
echo "${numxyz}" conformers detected in ./geometries/unoptimized/

cd ../.. # returns to regular working directory

# Exits script if template files don't exist or path to them can't be found
if [ ! -f "template_inppbs/template_opt.inp" ] || [ ! -f \
"template_inppbs/template_opt.pbs" ]; then
    echo
    echo The filepath is broken or file is missing for one or both of:
    echo
	echo ./template_inppbs/template_opt.inp
    echo ./template_inppbs/template_opt.pbs 
	echo
	exit
fi
...
echo
echo To generate opt.inp and opt.pbs files for talapas, 
echo choose a conformer library:
echo
echo NOTE: It is not recommended to generate opt.pbs and opt.inp files for
...
# character (-print0)
# the regex bit takes allows find to ignore hidden files and directories
	...
	case $opt in 
		*.csv) 
		...
		"Use all ${numxyz} conformers (not recommended)") 
			echo
			echo You chose to generate opt.inp and opt.pbs files for every \
            conformer.
...
if [ ${exitcode} = "reduced" ]; then
	...
	for conf in geometries/unoptimized/*.xyz; do
		conf="${conf##*/}"
		conf="${conf%.*}"
		[ -d "conformer_files/${conf}/" ] || mkdir "conformer_files/${conf}/"
	done
...
inptemplate="template_inppbs/template_opt.inp"
pbstemplate="template_inppbs/template_opt.pbs"

# opt.inp and opt.pbs file creation
echo Generating opt.inp and opt.pbs files for each conformer...
echo
for confdir in conformer_files/*; do
	confdir="${confdir##*files/}"
	confdir="${confdir%/*}"
	if [ ! -f "conformer_files/${confdir}/${confdir}_opt.inp" ] && \
    [ ! -f "conformer_files/${confdir}/${confdir}_opt.pbs" ]; then
		if [ ! -f "geometries/unoptimized/${confdir}.xyz" ]; then
			echo "ERROR: Missing file ./geometries/unoptimized/${confdir}.xyz"
		else
			cp -nv "${inptemplate}" \
            "conformer_files/${confdir}/${confdir}_opt.inp"

			cp -nv "${pbstemplate}" \
            "conformer_files/${confdir}/${confdir}_opt.pbs"

			cat "geometries/unoptimized/${confdir}.xyz" >> \
            "conformer_files/${confdir}/${confdir}_opt.inp" # entire .xyz file
            # is written to opt.inp

			gsed -i '10,14d' "conformer_files/${confdir}/${confdir}_opt.inp" 
            # removes lines 10 - 14

			echo -e '\n \n' >> "conformer_files/${confdir}/${confdir}_opt.inp"
            # two new lines are added to the end of opt.inp
            # (gaussian requirement)

			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}/${confdir}_opt.inp" # replaces all
            # instances of "PLACEHOLDER" with filename in opt.inp
            
			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}/${confdir}_opt.pbs" # replaces all
            # instances of "PLACEHOLDER" with filename in opt.pbs
		fi
	else
		echo "ERROR: Existing file(s) ${confdir}_opt.inp and/or" \
        "${confdir}_opt.pbs" # overwrite protection for existing
...
echo opt calculations are ready to submit to Talapas.
echo
