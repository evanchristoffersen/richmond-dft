...
if [ ! -f "template_inppbs/template_hf.inp" ] || [ ! -f \
"template_inppbs/template_hf.pbs" ]; then
    echo
    echo The filepath is broken or file is missing for one or both of:
    echo
	echo ./template_inppbs/template_hf.inp
    echo ./template_inppbs/template_hf.pbs 
	echo
	exit
fi
...
echo
echo To generate hf.inp and hf.pbs files for talapas, 
echo choose a conformer library:
echo
echo NOTE: It is not recommended to generate hf.pbs and hf.inp files for
	...
	case $opt in 
		*.csv) 
		...
		"Use all ${numxyz} conformers (not recommended)") 
			echo
			echo You chose to generate hf.inp and hf.pbs files for every \
            conformer.
...
inptemplate="template_inppbs/template_hf.inp"
pbstemplate="template_inppbs/template_hf.pbs"

# hf.inp and hf.pbs file creation
echo Generating hf.inp and hf.pbs files for each conformer...
echo
for confdir in ./conformer_files/*; do
	confdir="${confdir##*files/}"
	confdir="${confdir%/*}"
	if [ ! -f "conformer_files/${confdir}/${confdir}_hf.inp" ] && \
    [ ! -f "conformer_files/${confdir}/${confdir}_hf.pbs" ]; then
		if [ ! -f "geometries/optimized/${confdir}_opt.xyz" ]; then
			echo "ERROR: Missing file ./geometries/optimized/${confdir}_opt.xyz"
		else
			cp -nv "${inptemplate}" \
            "conformer_files/${confdir}/${confdir}_hf.inp"

			cp -nv "${pbstemplate}" \
            "conformer_files/${confdir}/${confdir}_hf.pbs"

			cat "geometries/optimized/${confdir}_opt.xyz" >> \
            "conformer_files/${confdir}/${confdir}_hf.inp" # entire .xyz file 
            # is written to hf.inp

			gsed -i '10,14d' "conformer_files/${confdir}/${confdir}_hf.inp"
            # removes lines 10 - 14

			echo -e '\n \n' >> "conformer_files/${confdir}/${confdir}_hf.inp" 
            # two new lines are added to the end of hf.inp
            # (gaussian requirement)

			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}/${confdir}_hf.inp" # replaces all 
            # instances of "PLACEHOLDER" with filename in hf.inp
            
			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}"/"${confdir}_hf.pbs" # replaces all 
            # instances of "PLACEHOLDER" with filename in hf.pbs
		fi
	else
		echo "ERROR: Existing file(s) ${confdir}_hf.inp and/or" \
        "${confdir}_hf.pbs" # overwrite protection for existing
...
echo hf calculations are ready to submit to Talapas.
echo
