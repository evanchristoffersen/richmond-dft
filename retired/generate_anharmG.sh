...
if [ ! -f "template_inppbs/template_anharm.inp" ] || [ ! -f \
"template_inppbs/template_anharm.pbs" ]; then
    echo
    echo The filepath is broken or file is missing for one or both of:
    echo
	echo ./template_inppbs/template_anharm.inp
    echo ./template_inppbs/template_anharm.pbs 
	echo
	exit
fi
...
echo
echo To generate anharm.inp and anharm.pbs files for talapas, 
echo choose a conformer library:
echo
echo NOTE: It is not recommended to generate anharm.pbs and anharm.inp files for
	...
	case $opt in 
		*.csv) 
		...
		"Use all ${numxyz} conformers (not recommended)") 
			echo
			echo You chose to generate anharm.inp and anharm.pbs files for every \
            conformer.
...
inptemplate="template_inppbs/template_anharm.inp"
pbstemplate="template_inppbs/template_anharm.pbs"

# anharm.inp and anharm.pbs file creation
echo Generating anharm.inp and anharm.pbs files for each conformer...
echo
for confdir in ./conformer_files/*; do
	confdir="${confdir##*files/}"
	confdir="${confdir%/*}"
	if [ ! -f "conformer_files/${confdir}/${confdir}_anharm.inp" ] && \
    [ ! -f "conformer_files/${confdir}/${confdir}_anharm.pbs" ]; then
		if [ ! -f "geometries/optimized/${confdir}_opt.xyz" ]; then
			echo "ERROR: Missing file ./geometries/optimized/${confdir}_opt.xyz"
		else
			cp -nv "${inptemplate}" \
            "conformer_files/${confdir}/${confdir}_anharm.inp"

			cp -nv "${pbstemplate}" \
            "conformer_files/${confdir}/${confdir}_anharm.pbs"

			cat "geometries/optimized/${confdir}_opt.xyz" >> \
            "conformer_files/${confdir}/${confdir}_anharm.inp" # entire .xyz file 
            # is written to anharm.inp

			gsed -i '10,14d' "conformer_files/${confdir}/${confdir}_anharm.inp"
            # removes lines 10 - 14

			echo -e '\n \n' >> "conformer_files/${confdir}/${confdir}_anharm.inp" 
            # two new lines are added to the end of anharm.inp
            # (gaussian requirement)

			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}/${confdir}_anharm.inp" # replaces all 
            # instances of "PLACEHOLDER" with filename in anharm.inp
            
			gsed -i "s/PLACEHOLDER/${confdir}/g" \
            "conformer_files/${confdir}"/"${confdir}_anharm.pbs" # replaces all 
            # instances of "PLACEHOLDER" with filename in anharm.pbs
		fi
	else
		echo "ERROR: Existing file(s) ${confdir}_anharm.inp and/or" \
        "${confdir}_anharm.pbs" # overwrite protection for existing
...
echo anharm calculations are ready to submit to Talapas.
echo
